        ...
        {result ? (
          <div className="card" style={{ padding: "2rem 1.75rem", textAlign: "center" }}>
            ...
            <p className="muted" style={{ marginBottom: "1.5rem" }}>
              Ini voucher kopi gratismu. Tunjukkan kode ini ke barista.
            </p>
            <div className="code">{result.voucher.code}</div>
            <p style={{ marginTop: "1.25rem", fontSize: "0.9rem" }}>
              {result.voucher.delivered
                ? "Kode juga sudah dikirim ke email kamu."
                : "Simpan / screenshot kode ini ya."}
            </p>
          </div>
        ) : (
          <>
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <h1 style={{ fontSize: "clamp(1.8rem, 5vw, 2.4rem)", margin: "0.5rem 0" }}>
                Daftar member, dapat <span className="outlined">kopi gratis</span>.
              </h1>
              <p className="muted">Cukup sekali isi. Datamu = voucher pertamamu.</p>
            </div>

            <form className="card" style={{ padding: "1.75rem 1.6rem" }} onSubmit={submit}>
              <div className="field">
                <label htmlFor="name">Nama *</label>
                <input id="name" className="input" required value={form.name} onChange={set("name")} placeholder="Nama kamu" />
              </div>
              <div className="field">
                <label htmlFor="phone">Nomor HP / WhatsApp *</label>
                <input id="phone" className="input" required value={form.phone} onChange={set("phone")} placeholder="08xxxxxxxxxx" inputMode="tel" />
              </div>
              <div className="field">
              ...
              <div className="field">
                ...
                <input id="ig" className="input" value={form.ig_handle} onChange={set("ig_handle")} placeholder="@username (opsional)" />
              </div>

              {error && <div className="notice notice--err" style={{ marginBottom: "1rem" }}>{error}</div>}

              <button className="btn btn--primary btn--block" disabled={loading} type="submit">
                {loading ? "Mendaftar…" : "Daftar & ambil voucher"}
              </button>
              <p className="muted" style={{ fontSize: "0.78rem", marginTop: "0.9rem", textAlign: "center" }}>
                Dengan mendaftar, kamu setuju datamu dipakai Tana Arabica untuk promo &amp; loyalty.
              </p>
            </form>
          </>
        )}
